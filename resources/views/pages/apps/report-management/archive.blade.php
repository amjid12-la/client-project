    ...
    <div class="card">
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                ...
                <h3 class="fw-bold m-0">Rejected Reports Archive ({{ $rejectedReports->count() }})</h3>
            </div>
            <div class="card-toolbar">
                ...
                <div class="d-flex align-items-center me-3">
                    <div class="position-relative">
                        ...
                        <input type="text" id="archive-search" class="form-control form-control-sm ps-10 pe-10" placeholder="Search reports..." style="min-width: 250px;">
                        <button type="button" id="archive-search-clear" class="btn btn-sm btn-icon position-absolute end-0 top-50 translate-middle-y me-1" style="display: none;">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.reports.approved') }}" class="btn btn-sm btn-light-success">
        ...
        <div class="card-body py-4">
            @if($rejectedReports->isEmpty())
                <div class="text-center py-10">
                    ...
                    <h3 class="text-muted">No Rejected Reports</h3>
                    <p class="text-muted">No reports have been rejected yet.</p>
                </div>
            @else
                <div class="table-responsive">
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="archive-reports-table">
                        <thead>
                            <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-100px">Rejected</th>
                        ...
                        <tbody class="text-gray-600 fw-semibold">
                            @foreach($rejectedReports as $report)
                                <tr>
                                    ...
                                    <td data-order="{{ $report->reviewed_at->timestamp }}">
                                        <span class="badge badge-light-danger">{{ $report->reviewed_at->diffForHumans() }}</span>
                                    </td>
                                    <td>{{ Str::words($report->reviewer->name ?? 'N/A', 4, '...') }}</td>
                                    <td class="text-end">
                                        <button type="button" class="btn btn-sm btn-light btn-active-light-primary me-2 view-report-btn"
                                                ...
                                                data-report-reviewed="{{ $report->reviewed_at->format('M d, Y h:i A') }}"
                                        ...
                                        <form action="{{ route('admin.reports.destroy', $report->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-light-danger btn-active-danger" onclick="return confirm('Are you sure you want to delete this report?')">
                                                {!! getIcon('trash', 'fs-3') !!}
                                                Delete
                                            </button>
                                        </form>
                ...
                <style>
                    ...
                    #archive-reports-table thead th {
                        padding-bottom: 16px !important;
                    }
                    
                    #archive-reports-table tbody td {
                        padding-top: 16px !important;
                    }
                    
                    /* Force left alignment for all cells except Actions */
                    #archive-reports-table td,
                    #archive-reports-table th {
                        text-align: left !important;
                    }
                    
                    #archive-reports-table td:last-child,
                    #archive-reports-table th:last-child {
                    ...
                    .dataTables_wrapper .dataTables_filter input:focus {
                        border-color: #f1416c;
                        box-shadow: 0 0 0 0.2rem rgba(241, 65, 108, 0.1);
                    }
                ...
                <script>
                    $(document).ready(function() {
                        var table = $('#archive-reports-table').DataTable({
                            order: [[4, 'desc']], // Sort by rejected date descending - newest first (changed from 3 to 4)
                            pageLength: 10,
                            ...
                            drawCallback: function() {
                                ...
                                $('html, body').animate({
                                    scrollTop: $('#archive-reports-table').offset().top - 100
                                }, 300);
                                
                                // Force left alignment after each draw for both headers and data
                                $('#archive-reports-table th, #archive-reports-table td').css('text-align', 'left');
                                $('#archive-reports-table th:first-child, #archive-reports-table td:first-child').css('text-align', 'center');
                                $('#archive-reports-table th:last-child, #archive-reports-table td:last-child').css('text-align', 'right');
                            }
                        });
                        ...
                        $('#archive-search').on('keyup', function() {
                            ...
                            if (searchValue.length > 0) {
                                $('#archive-search-clear').show();
                            } else {
                                $('#archive-search-clear').hide();
                            }
                        });

                        // Clear search button
                        $('#archive-search-clear').on('click', function() {
                            $('#archive-search').val('');
                            table.search('').draw();
                            $(this).hide();
                        });
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5" id="modal-photo-container">
                            <div class="mb-3">
                                ...
                                <div class="text-center">
                                    <img id="modal-photo" src="" alt="Report Photo" class="img-fluid rounded shadow-sm" style="max-height: 500px; width: 100%; object-fit: cover; border: 3px solid #e9ecef;">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-7">
                            ...
                            <div class="mb-3">
                                ...
                                <p id="modal-narrative" class="text-gray-800" style="white-space: pre-wrap; line-height: 1.6;"></p>
                            </div>
                            ...
                            <!-- Reviewed Info (for rejected) -->
                            <div id="modal-reviewed-container" style="display: none;">
                                <div class="mb-3">
                                    <label class="fw-bold fs-6 mb-2 text-danger">
                                        {!! getIcon('cross-circle', 'fs-3 me-2') !!}Rejected At
                                    </label>
                                    <p id="modal-reviewed" class="text-gray-800"></p>
                                </div>
                                <div class="mb-3">
                                    <label class="fw-bold fs-6 mb-2 text-danger">
                                        {!! getIcon('user', 'fs-3 me-2') !!}Rejected By
                                    </label>
